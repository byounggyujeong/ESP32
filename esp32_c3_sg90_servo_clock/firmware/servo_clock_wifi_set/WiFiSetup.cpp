...
static String buildPortalPage() {
  String html = F(
    ...
    "<meta charset='UTF-8'>"
    "<title>ServoClock WiFi Setup</title>"
    "<style>"
...
// 저장 + 연결 시도까지 한번에 처리 (AP는 유지한 채로 STA 동시 연결)
static void handleSave() {
  newSSID = server.arg("ssid");
  newPASS = server.arg("pass");
  saveCredentials(newSSID, newPASS);

  // AP는 유지하면서 STA로 동시 연결 시도
  WiFi.mode(WIFI_AP_STA);
  showStatus("Saved! Connecting", newSSID.c_str());
  connectSuccess = tryConnect(newSSID, newPASS, 15);
  ...
  String resp = F(
    ...
    "a{color:#4a90d9;}</style></head><body>"
  );

  if (connectSuccess) {
    resp += "<h2>✅ 연결 성공</h2>";
    resp += "<p>IP 주소:<br><b style='font-size:20px;'>" + WiFi.localIP().toString() + "</b></p>";
    resp += "<p>핸드폰을 원래 WiFi로 다시 연결한 뒤<br>";
    resp += "<a href='http://" + WiFi.localIP().toString() + "'>http://" + WiFi.localIP().toString() + "</a><br>";
    resp += "또는 <a href='http://servoclock.local'>http://servoclock.local</a> 로 접속하세요.</p>";
  } else {
    resp += "<h2>❌ 연결 실패</h2>";
    resp += "<p>SSID/비밀번호를 다시 확인 후<br>이 페이지에서 재시도해주세요.</p>";
  }

  resp += F("</body></html>");
  server.send(200, "text/html", resp);

  credentialsReceived = true;   // 메인 루프에 처리 완료 신호
}
...
static bool runConfigPortal(const char* apName, const char* apPassword,
  ...
  while (true) {
    ...
    // 사용자가 저장 버튼 누름 → handleSave()에서 이미 연결 시도까지 완료된 상태
    if (credentialsReceived) {
      if (connectSuccess) {
        // 연결 성공 → 응답 페이지가 이미 IP를 보여줬으니 AP 종료하고 정상 진행
        ...
        // 연결 실패 → AP만 유지하고 STA 끈 채로 포털에서 재시도 대기
        WiFi.mode(WIFI_AP);
        credentialsReceived = false;
        Serial.println("[WiFiSetup] 연결 실패 → 포털에서 재시도 대기 중...");
      }
    }
